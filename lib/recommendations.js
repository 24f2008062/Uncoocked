...
export async function getRecommendedEvents(userEmail) {
  ...
  const scoredEvents = events.map(event => {
    ...
    // 1. Interest Match (40%)
    let interestScore = 0;
    const isCategoryMatch = event.category && userInterests.includes(event.category);
    ...
    const hasInterestInTags = eventTags.some(tag => userInterests.some(i => i.toLowerCase() === tag.toLowerCase()));
    const hasInterestInKeywords = eventKeywords.some(kw => userInterests.some(i => i.toLowerCase() === kw.toLowerCase()));
    
    if (isCategoryMatch) interestScore += 100;
    else if (hasInterestInTags || hasInterestInKeywords) interestScore += 70;
