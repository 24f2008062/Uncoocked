...
interface BulletinBoardProps {
  ...
  onAddUpdate?: (newUpdate: { title: string; content: string }) => void;
}

export default function BulletinBoard({ updates, onAddUpdate }: BulletinBoardProps) {
  const [modalOpen, setModalOpen] = useState(false);
  const [targetUpdate, setTargetUpdate] = useState<BulletinUpdate | null>(null);
  ...
  function openConversion(u: BulletinUpdate) {
    setTargetUpdate(u);
    setModalOpen(true);
  }
  ...
  return (
    <div className="bg-dark-card rounded-lg border border-dark-border overflow-hidden shadow-neon space-y-4 p-1 animate-fadeIn h-full flex flex-col">
      ...
      {onAddUpdate && showAddForm && (
        <form onSubmit={handleFormSubmit} className="px-6 py-4 border border-neon-purple/35 rounded-md bg-black mx-2 space-y-3 flex-shrink-0">
          <h4 className="text-xs font-bold text-neon-purple uppercase tracking-wider">Broadcast New Update</h4>
          
          <div>
            <label htmlFor="broadcast-title" className="block text-[10px] font-semibold text-gray-400">Update Title</label>
            <input
              id="broadcast-title"
              required
              type="text"
          ...
          <div>
            <label htmlFor="broadcast-content" className="block text-[10px] font-semibold text-gray-400">Message Content</label>
            <textarea
      ...
      <div className="bg-black px-6 py-3 border-t border-dark-border rounded-b-md">
        <p className="text-[10px] text-gray-400 leading-normal">
          💡 {onAddUpdate ? 'You are acting as an Organizer. Broadcasts will refresh in this sandbox.' : 'This board is managed by organizers. Comments and user submissions are not allowed.'}
        </p>
      </div>
