...
export interface Event {
  ...
  schedule?: string;
  prizePool?: string;
  bulletinUpdates?: { id: string; date: string; title: string; content: string }[];
  bannerUrl?: string;
...
- **🎸 Battle of Bands Winner** - $3,000
    `,
    bulletinUpdates: [
      { id: 'u1', date: '2026-06-16', title: 'Registration Deadline Extended!', content: 'Contingent registration is open until June 18th.' },
      { id: 'u2', date: '2026-06-15', title: 'Celebrity Guest Confirmed', content: 'Rock star VIP guest lineup is locked for the final night performance!' }
    ],
    ticketType: 'Paid',
    price: 49.99,
    capacity: 2000,
...
- **🌟 Best Talent Performer** - $500 Voucher
    `,
    bulletinUpdates: [
      { id: 'u3', date: '2026-06-16', title: 'Dress Code Announced', content: 'The theme is Retro Neon. Come dressed in neon colors!' }
    ],
    ticketType: 'Free',
    capacity: 1500,
...
- **Category Winners** - $5,000 each (Mobile, AI/ML, Design)
    `,
    bulletinUpdates: [
      { id: 'u4', date: '2026-06-16', title: 'Hackathon Registration Now Open!', content: 'Team registration is now live. Sign up by June 18th.' },
      { id: 'u5', date: '2026-06-15', title: 'Mentor List Released', content: 'Meet our amazing panel of mentors from top tech companies.' }
    ],
    ticketType: 'Free',
    capacity: 300,
    waitlistEnabled: true
...
export default function EventsExplorer({
  ...
  const filteredEvents = useMemo(() => {
    return events.filter((ev) => {
      const matchCategory = activeCategory === 'All' || ev.type === activeCategory;
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b border-dark-border pb-8">
        <div className="space-y-2 text-center md:text-left">
          ...
          <p className="text-xs text-gray-400 max-w-xl leading-relaxed">
            Discover active coding hackathons, security CTF challenges, business ideathons, and legislative debates.
          </p>
        </div>
            ...
            >
              ...
              <div className="relative h-32 w-full overflow-hidden bg-zinc-900 border-b border-dark-border/40">
                ...
                <div className="absolute top-3 left-3">
                  <span className={`text-[9px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-full ${getTypeStyle(ev.type)}`}>
                    {ev.type}
                  </span>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center gap-4 text-[10px] text-gray-500 font-mono">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="truncate max-w-[140px]">{ev.location.split(',')[0]}</span>
                    </div>
                  </div>
