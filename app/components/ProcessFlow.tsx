...
export default function ProcessFlow() {
  const steps = [
    { number: '01', title: 'Event Discovery', desc: 'Find campus fests, parties, dandiya nights, workshops, or hackathons.' },
    { number: '02', title: 'Community Join', desc: 'Gain immediate access to secure, noise-free local channels.' },
    { number: '03', title: 'Find Collaborators', desc: 'Browse student profiles, majors, and past status bios.' },
    { number: '04', title: 'Secure Entry', desc: 'Secure tickets and register on guest lists instantly.' },
    { number: '05', title: 'Uncooked Projects', desc: 'Unlock club roles, leadership grants, and campus show calls.' },
  ];
    ...
    <section className="w-full py-16 bg-black border-t border-dark-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-12">
        <div className="space-y-4 max-w-2xl mx-auto">
          <span className="text-xs font-extrabold text-neon-purple tracking-widest uppercase block neon-text-glow">The Student Pipeline</span>
          <h2 className="text-3xl font-black text-white">How the Journey Flows</h2>
          <p className="text-xs text-gray-400">
            A seamless horizontal progression from discovering campus events to coordinating student check-ins.
          </p>
        </div>
          ...
          {steps.map((step, idx) => (
            <div key={idx} className="space-y-4 group">
              ...
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-white group-hover:text-neon-purple transition-colors">{step.title}</h4>
