...
export default function RegistrationDatabase() {
  ...
  const [sortField, setSortField] = useState<'name' | 'ts'>('ts');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        ...
        const enriched: Registration[] = stored.map((reg: any, idx: number) => {
          const statuses: ('Confirmed' | 'Pending' | 'Checked In')[] = ['Confirmed', 'Pending', 'Checked In'];
          const tracks = ['Frontend Dev', 'Backend Engineer', 'Fullstack Developer', 'UI/UX Designer', 'AI Specialist'];
          
          return {
            ...reg,
            status: reg.status || statuses[idx % statuses.length],
            track: reg.track || tracks[idx % tracks.length],
            github: reg.github || `https://github.com/${reg.name.toLowerCase().replace(/\s+/g, '')}`,
          };
        });
  ...
  const handleRowClick = (email: string, eventId: string) => {
  ...
  const handleSort = (field: 'name' | 'ts') => {
    if (sortField === field) {
                    ...
                    return (
                      <React.Fragment key={rowKey}>
                        ...
                        >
                          ...
                          <td className="py-4 px-6 whitespace-nowrap">
                            <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider ${getStatusStyle(reg.status)}`}>
                              {reg.status}
                            </span>
                          </td>
                        </tr>
                        ...
                        {isExpanded && (
                          <tr className="bg-black/30 border-l-2 border-l-neon-purple animate-fadeIn">
                            <td colSpan={4} className="py-5 px-6 border-b border-dark-border/40">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-[11px]">
                                ...
                                <div className="space-y-1.5">
                                  ...
                                  <p className="text-white font-bold pl-5">{reg.track || 'Unassigned Department'}</p>
                                </div>
                                ...
                                <div className="space-y-1.5">
                                  ...
                                  <p className="text-white font-bold pl-5">{reg.team || 'Individual'}</p>
                                </div>
                                ...
                                <div className="space-y-1.5">
                                  ...
                                  <p className="text-white pl-5">{reg.ts ? new Date(reg.ts).toLocaleDateString() : '—'}</p>
                                </div>
                                ...
                                <div className="space-y-1.5">
                                  ...
                                  <p className="text-white pl-5">
                                    {reg.ts ? new Date(reg.ts).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'}
                                  </p>
                                </div>
                              </div>
                            </td>
