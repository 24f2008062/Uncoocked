...
import BulletinBoard, { BulletinUpdate } from './BulletinBoard';
...
export default function TwinLayout({ event, onBack }: TwinLayoutProps) {
  ...
  const [bulletins, setBulletins] = useState<BulletinUpdate[]>(event.bulletinUpdates);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        ...
        const filtered = stored.filter((r: Registration) => r.eventId === event.id);
  ...
  async function handleRegister(payload: { name: string; email: string; team?: string }) {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        const currentTicketsSold = stored.filter((r: Registration) => r.eventId === event.id && r.status !== 'Waitlisted').length;
  ...
  const handleCancelRegistration = (eventId: string) => {
    if (typeof window !== 'undefined') {
      try {
        const stored = JSON.parse(localStorage.getItem('registrations') || '[]');
        const activeEmail = user || localUserEmail || (userRegistration && userRegistration.email);
        if (activeEmail) {
          const updated = stored.filter((reg: Registration) => !(reg.email === activeEmail && reg.eventId === eventId));
  ...
  const handleAddUpdate = (newUpdate: { title: string; content: string }) => {
    const fresh: BulletinUpdate = {
      id: `update-${Date.now()}`,
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (storedHostedStr) {
          ...
          const updated = hosted.map((hEv: any) => {
            if (hEv.id === event.id) {
              ...
              <div className="bg-dark-card rounded-lg p-6 border border-dark-border shadow-neon">
                ...
                <div className="prose dark:prose-invert max-w-none">
                  <ReactMarkdown
                    components={{
                      h3: ({ node: _node, ...props }) => (
                        <h3 className="text-xs font-bold uppercase tracking-wider text-white mt-4 mb-2 font-mono" {...props} />
                      ),
                      p: ({ node: _node, ...props }) => (
                        <p className="text-gray-300 text-[11px] mb-2" {...props} />
                      ),
                      ul: ({ node: _node, ...props }) => (
                        <ul className="list-disc list-inside text-gray-300 text-[11px] space-y-1 ml-2" {...props} />
                      ),
                      li: ({ node: _node, ...props }) => (
                        <li className="text-gray-300 text-[11px]" {...props} />
                      ),
                      strong: ({ node: _node, ...props }) => (
                        <strong className="font-bold text-neon-purple font-mono" {...props} />
                      ),
                    }}
                  >
                    {event.schedule}
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="flex flex-col rounded-lg bg-dark-card border border-dark-border p-6 space-y-4 shadow-neon">
                ...
                {isRegistered ? (
                  <div className="space-y-4">
                    <div className={`p-4 border rounded-lg space-y-2 ${userRegistration.status === 'Waitlisted' ? 'bg-yellow-950/20 border-yellow-800/40' : 'bg-emerald-950/20 border-emerald-800/40'}`}>
                      <div className={`flex items-center gap-2 text-xs font-bold uppercase ${userRegistration.status === 'Waitlisted' ? 'text-yellow-400' : 'text-emerald-400'}`}>
                        ...
                        {userRegistration.status === 'Waitlisted' ? 'On Waitlist' : 'Ticket Secured'}
                      </div>
                      <div className="text-[11px] text-gray-300 font-mono space-y-1 leading-normal pt-1">
                        <div><span className="text-gray-500">Name:</span> {userRegistration.name}</div>
                        <div><span className="text-gray-500">Email:</span> {userRegistration.email}</div>
                        {userRegistration.ticketId && (
                          <div><span className="text-gray-500">Ticket ID:</span> {userRegistration.ticketId}</div>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {!isSoldOut || isWaitlistOnly ? (
                      user ? (
                        ...
                        >
                          {isWaitlistOnly ? '🕒 Join Waitlist' : (event.ticketType === 'Paid' ? `🛒 Buy Ticket - $${event.price}` : '🎫 Secure Free Ticket')}
                        </button>
                      ) : (
                        <button 
              ...
              {user && role === 'organizer' && event.hostEmail === user && (
                <div className="bg-dark-card border border-dark-border rounded-lg p-6 space-y-4 shadow-neon">
                  <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2 border-b border-dark-border pb-3">
                    <span className="text-neon-purple">⚡</span> Organizer Shortcuts
                  </h3>
                  <p className="text-xs text-gray-400 leading-relaxed">
                    Student booking guest lists, check-in sheets, and hosted activities are managed inside the console.
                  </p>
                  <Link 
                    href="/dashboard" 
                  ...
                  <form onSubmit={handleFormSubmit} className="pt-4 border-t border-dark-border/40 space-y-3">
                    <h4 className="text-[10px] font-bold text-neon-purple uppercase tracking-wider">Broadcast New Update</h4>
                    
                    <div className="space-y-1">
                      <label htmlFor="right-broadcast-title" className="block text-[9px] font-semibold text-gray-400">Update Title</label>
                      <input
                    ...
                    <div className="space-y-1">
                      <label htmlFor="right-broadcast-content" className="block text-[9px] font-semibold text-gray-400">Message Content</label>
                      <textarea
