...
export default function BuilderNetwork() {
  const leftNodes = [
    { name: 'CS & Tech', count: 340, icon: Layout, color: 'text-blue-400', desc: 'Computers, Coding, IT' },
    { name: 'Engineering', count: 290, icon: Server, color: 'text-green-400', desc: 'Mechanical, EEE, Applied' },
    { name: 'Business', count: 420, icon: Cpu, color: 'text-neon-purple', desc: 'Finance, Marketing, Econ' },
  ];

  const rightNodes = [
    { name: 'Liberal Arts', count: 210, icon: Brain, color: 'text-yellow-400', desc: 'Humanities, Literature' },
    { name: 'Fine Arts', count: 180, icon: Palette, color: 'text-pink-400', desc: 'Design, Painting, Music' },
    { name: 'Natural Sciences', count: 120, icon: Briefcase, color: 'text-cyan-400', desc: 'Physics, Chemistry, Bio' },
  ];
    ...
    <section className="py-16 relative w-full border-t border-white/5 bg-zinc-950/20 overflow-hidden">
      <div className="mx-auto max-w-7xl px-6 lg:px-8 space-y-12 text-center relative">
        ...
        <div className="space-y-3">
          <span className="text-xs font-extrabold text-neon-purple tracking-widest uppercase block neon-text-glow">Campus Ecosystem</span>
          <h2 className="text-3xl font-black text-white tracking-tight sm:text-4xl">The Unified Network</h2>
          <p className="text-xs text-gray-400 max-w-lg mx-auto leading-normal">
            We connect student attendees across active majors and branches of study to help societies discover talent, organize fests, and balance event check-ins.
          </p>
        </div>
        ...
        <div className="relative max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-12 md:gap-4 py-8">
          ...
          <div className="flex flex-col gap-6 w-full md:w-52 text-left z-10">
            {leftNodes.map((node, i) => {
              ...
              return (
                ...
                >
                  <div className={`w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0 ${node.color}`}>
                    <Icon className="h-4 w-4" />
                  </div>
                  <div>
                    <span className="block text-[11px] font-extrabold text-white uppercase">{node.name}</span>
                    <span className="text-[9px] text-gray-500">{node.count} students</span>
                  </div>
                </motion.div>
              );
            })}
          ...
          <div className="relative flex items-center justify-center py-6">
            ...
            >
              ...
              <span className="text-[10px] font-black tracking-widest text-white uppercase font-mono mt-1 leading-none">Campus</span>
              <span className="text-[10px] font-black tracking-widest text-white uppercase font-mono leading-none">Students</span>
              <span className="text-[8px] font-bold text-neon-lavender font-mono mt-1">1,240 CORE</span>
            </motion.div>
          </div>
