...
export default function EventMatrixPreview() {
      ...
      try {
        ...
        if (userEmail) {
          const matchingIds = stored
            .filter((r: any) => r.email === userEmail)
            .map((r: any) => r.eventId);
  ...
  const handleOpenRegister = (ev: MockEvent) => {
    setSelectedEvent(ev);
    setModalOpen(true);
  };

  const handleRegisterSubmit = async (payload: { name: string; email: string; team?: string }) => {
    ...
    try {
      ...
      const alreadyRegistered = storedRegs.some(
        (reg: any) => reg.email === payload.email && reg.eventId === selectedEvent.id
      );
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-3">
            <span className="text-xs font-extrabold text-neon-purple tracking-widest uppercase block neon-text-glow">Event Core</span>
            <h2 className="text-3xl font-black text-white tracking-tight sm:text-4xl">Explore the Event Matrix</h2>
            <p className="text-xs text-gray-400 max-w-lg leading-normal">
              Browse and checkout tickets for upcoming hackathons, startup pitch panels, system dev sprints, and community drives.
            </p>
          </div>
                  ...
                  <div className="relative h-36 w-full overflow-hidden bg-zinc-900 border-b border-white/5">
                    ...
                    <div className="absolute top-3 left-3">
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider bg-zinc-950/80 border border-white/10 text-gray-300 font-mono">
                        <Tag className="h-2.5 w-2.5 text-neon-purple" /> {ev.category}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-5 flex flex-col justify-between flex-1 space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-2 text-[10px] font-mono border-t border-white/5 text-gray-500">
                        <div>
                          <span className="block text-[8px] uppercase tracking-wider text-gray-600">Prize Pool</span>
                          <span className="text-white font-bold">{ev.prizePool}</span>
                        </div>
                        <div>
                          <span className="block text-[8px] uppercase tracking-wider text-gray-600">Registrations</span>
                          <span className="text-white font-bold">{ev.registrations} secured</span>
                        </div>
                      </div>
                    </div>
