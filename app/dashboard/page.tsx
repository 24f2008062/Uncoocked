...
import { mockEvents, Event as CampusEvent } from '@/app/components/EventsExplorer';
import { 
  Calendar, 
  MapPin, 
  Briefcase, 
...
const getInitialHostedEvents = (email: string): HostedEvent[] => [
  {
    id: 'hosted-ev-1',
    ...
    hostEmail: email,
    bulletinUpdates: [
      { id: 'hu-1', date: '2026-06-17', title: 'Speaker Schedule Released', content: 'The full panel timing is now published.' }
    ],
...
const BANNER_PRESETS = [
  { name: 'Hackathon', url: 'https://images.unsplash.com/photo-1504384308090-c894fdcc538d?w=600&auto=format&fit=crop&q=60' },
  { name: 'Workshop', url: 'https://images.unsplash.com/photo-1531482615713-2afd69097998?w=600&auto=format&fit=crop&q=60' },
  { name: 'Festive Night', url: 'https://images.unsplash.com/photo-1605649487212-47bdab064df7?w=600&auto=format&fit=crop&q=60' },
  { name: 'Concert/Party', url: 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=600&auto=format&fit=crop&q=60' },
  { name: 'Meetup', url: 'https://images.unsplash.com/photo-1511578314322-379afb476865?w=600&auto=format&fit=crop&q=60' }
];
...
export default function DashboardPage() {
  ...
  const [allEvents, setAllEvents] = useState<(CampusEvent | HostedEvent)[]>([]);
  ...
  const [newTicketType, setNewTicketType] = useState<'Free' | 'Paid'>('Free');
  ...
  const [selectedEventForTicket, setSelectedEventForTicket] = useState<(CampusEvent | HostedEvent) | null>(null);
  ...
  const [expandedGuestLists, setExpandedGuestLists] = useState<{ [key: string]: boolean }>({});
  // Announcement Input States
  const [announcementTitles, setAnnouncementTitles] = useState<{ [key: string]: string }>({});
  const [announcementContents, setAnnouncementContents] = useState<{ [key: string]: string }>({});
    ...
    if (typeof window !== 'undefined' && user) {
      try {
        ...
        if (!storedHostedStr) {
          ...
          hosted = parsed.filter((ev: HostedEvent) => ev.hostEmail === user);
        }
        ...
        hosted.forEach((hEv) => {
          const hasRegs = storedRegs.some((r: any) => r.eventId === hEv.id);
          if (!hasRegs) {
            regSeeded = true;
            if (hEv.id === 'hosted-ev-1') {
              updatedRegs.push(
                { name: 'Emily Davis', email: 'emily@campus.edu', eventId: hEv.id, ts: Date.now() - 86400000, status: 'Confirmed', track: 'Fine Arts & Design', team: 'Creative Arts Society' },
                { name: 'Michael Chen', email: 'michael@campus.edu', eventId: hEv.id, ts: Date.now() - 43200000, status: 'Checked In', track: 'Computer Science & Tech', team: 'Tech Club' }
              );
            } else if (hEv.id === 'hosted-ev-2') {
              updatedRegs.push(
                { name: 'Sarah Connor', email: 'sarah@campus.edu', eventId: hEv.id, ts: Date.now() - 120000000, status: 'Confirmed', track: 'Engineering & Applied Sciences', team: 'Robotics Club' },
                { name: 'John Connor', email: 'john@campus.edu', eventId: hEv.id, ts: Date.now() - 60000000, status: 'Confirmed', track: 'Natural Sciences', team: 'Astronomy Club' }
              );
            }
          }
        });
        ...
        const userRegs = updatedRegs.filter((reg: Registration) => reg.email === user);
        setRegistrations(userRegs);

        // 4. Merge mockEvents and all hosted events to resolve event details
        const allHosted = JSON.parse(localStorage.getItem('hosted_events') || '[]');
  ...
  const handleCancelRegistration = (eventId: string) => {
    if (typeof window !== 'undefined' && user) {
      try {
        ...
        const updated = stored.filter((reg: Registration) => !(reg.email === user && reg.eventId === eventId));
  ...
  const handleEditHostedEventClick = (e: React.MouseEvent, hev: HostedEvent) => {
    e.stopPropagation();
    setIsEditing(true);
  ...
  const handleHostNewEvent = (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && editingEventId) {
        // Edit Mode
        const updated = storedHosted.map((hev: HostedEvent) => {
        ...
        const storedRegs = JSON.parse(localStorage.getItem('registrations') || '[]');
        storedRegs.push(
          { name: 'Alice Smith', email: 'alice@campus.edu', eventId: newEv.id, ts: Date.now(), status: 'Confirmed', track: 'Computer Science & Tech', team: 'Individual' },
          { name: 'Bob Johnson', email: 'bob@campus.edu', eventId: newEv.id, ts: Date.now(), status: 'Pending', track: 'Engineering & Applied Sciences', team: 'Tech Club' }
        );
  ...
  const handleCancelHostedEvent = (eventId: string) => {
    if (typeof window !== 'undefined' && user) {
      if (!confirm('Are you sure you want to cancel and delete this hosted event? All student bookings will be removed.')) return;
      try {
        // Remove from hosted events
        const storedHosted = JSON.parse(localStorage.getItem('hosted_events') || '[]');
        const updatedHosted = storedHosted.filter((ev: HostedEvent) => ev.id !== eventId);
        localStorage.setItem('hosted_events', JSON.stringify(updatedHosted));

        // Remove registrations for this event
        const storedRegs = JSON.parse(localStorage.getItem('registrations') || '[]');
        const updatedRegs = storedRegs.filter((reg: Registration) => reg.eventId !== eventId);
        localStorage.setItem('registrations', JSON.stringify(updatedRegs));
  ...
  const handleAddAnnouncement = (eventId: string) => {
    ...
    try {
      const storedHosted = JSON.parse(localStorage.getItem('hosted_events') || '[]');
      const updatedHosted = storedHosted.map((ev: HostedEvent) => {
        if (ev.id === eventId) {
  ...
  const getGuestList = (eventId: string) => {
    if (typeof window !== 'undefined') {
      try {
        const storedRegs = JSON.parse(localStorage.getItem('registrations') || '[]');
        return storedRegs.filter((reg: Registration) => reg.eventId === eventId);
      } catch {
        return [];
      }
    }
    ...
    <div className="relative isolate overflow-hidden bg-black w-full min-h-[85vh] py-12 sm:py-16">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-dark-border pb-8">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1 leading-normal">
              Manage your active event bookings, view details for hosted events, and post bulletin updates.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="space-y-6">
            <div className="bg-dark-card border border-dark-border rounded-xl p-5 shadow-neon">
              <h2 className="text-sm font-bold text-white uppercase tracking-wider mb-1 flex items-center gap-2 font-mono">
                <span className="text-neon-purple">🎫</span> Events You Are Attending
              </h2>
              <p className="text-xs text-gray-400">
                Official bookings and timelines for your registered fests, parties, and workshops.
              </p>
            </div>
            ...
            {loading ? (
              <div className="space-y-4">
                {[1, 2].map((n) => (
                  <div key={n} className="bg-dark-card border border-dark-border rounded-xl p-6 animate-pulse h-36" />
                ))}
              </div>
                    ...
                    >
                      <div className="space-y-2">
                        <div className="flex justify-between items-start gap-4">
                          ...
                          <span className={`text-[9px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${getStatusStyle(regDetails?.status)}`}>
                      ...
                      {/* Actions */}
                      <div className="flex items-center gap-3 pt-3 border-t border-dark-border/40 mt-3" onClick={(e) => e.stopPropagation()}>
                        <Link
                          href="/opportunities"
                        ...
                        >
                          ...
                          <span>Opportunit<span className="hidden sm:inline">ies</span></span>
                        </Link>
                        
                        {(regDetails?.status === 'Confirmed' || regDetails?.status === 'Checked In') && regDetails?.ticketId && (
                          <button
                            onClick={() => {
                        ...
                        <button
                          ...
                          title={regDetails?.status === 'Waitlisted' ? 'Leave Waitlist' : 'Cancel'}
                        >
          ...
          <div className="space-y-6">
            <div className="bg-dark-card border border-dark-border rounded-xl p-5 shadow-neon flex items-center justify-between">
              <div>
                <h2 className="text-sm font-bold text-white uppercase tracking-wider mb-1 flex items-center gap-2 font-mono">
                  <span className="text-neon-purple">📢</span> Events You Are Hosting
                </h2>
                <p className="text-xs text-gray-400">
                  Manage guest lists, edit descriptions, and publish event announcements.
                </p>
              </div>
              <button
                ...
                {hostedEvents.map((hev) => {
                  ...
                  return (
                    <div 
                      ...
                      onClick={() => router.push(`/event?id=${hev.id}`)}
                    >
                      ...
                      <div className="p-5 space-y-4">
                        ...
                        <div className="flex justify-between items-start gap-4">
                          ...
                          <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                            <button
                              onClick={(e) => handleEditHostedEventClick(e, hev)}
                        ...
                        <div className="flex items-center gap-2 pt-2 border-t border-dark-border/40" onClick={(e) => e.stopPropagation()}>
                          <button
                            onClick={() => setExpandedGuestLists(prev => ({ ...prev, [hev.id]: !prev[hev.id] }))}
                          ...
                          >
                            ...
                            {isGuestListExpanded ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                          </button>

                          <button
                            onClick={() => setExpandedAnnouncements(prev => ({ ...prev, [hev.id]: !prev[hev.id] }))}
                            className={`flex-1 py-1.5 px-2.5 rounded-lg border text-[11px] font-bold flex items-center justify-center gap-1.5 transition-all ${
                              isAnnounceExpanded
                                ? 'bg-neon-purple/10 border-neon-purple text-neon-lavender'
                                : 'bg-neutral-900 border-dark-border text-gray-400 hover:text-white'
                          ...
                          >
                            ...
                            <span>Announce</span>
                            {isAnnounceExpanded ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                          </button>
                        </div>

                        {/* Expandable Guest List */}
                        {isGuestListExpanded && (
                          <div className="bg-black/30 rounded-lg p-3.5 border border-dark-border/50 animate-fadeIn space-y-3" onClick={(e) => e.stopPropagation()}>
                            <h4 className="text-[10px] uppercase font-bold text-gray-400 flex items-center gap-1.5 font-mono">
                              <span>👥</span> Event Guest List
                            </h4>
                            {guestList.length === 0 ? (
                              <p className="text-[10px] text-gray-600 italic">No registrations for this event yet.</p>
                            ) : (
                              <div className="divide-y divide-dark-border/30 max-h-40 overflow-y-auto no-scrollbar">
                                {guestList.map((r: Registration, idx: number) => (
                                  <div key={idx} className="py-2 flex items-center justify-between text-[11px] font-mono">
                                    <div>
                        ...
                        {isAnnounceExpanded && (
                          <div className="bg-black/30 rounded-lg p-3.5 border border-dark-border/50 animate-fadeIn space-y-4" onClick={(e) => e.stopPropagation()}>
                            <h4 className="text-[10px] uppercase font-bold text-gray-400 flex items-center gap-1.5 font-mono">
                            ...
                            <div className="space-y-2">
                              <input
                                ...
                                onChange={(e) => setAnnouncementTitles(prev => ({ ...prev, [hev.id]: e.target.value }))}
                              ...
                              <textarea
                                ...
                                onChange={(e) => setAnnouncementContents(prev => ({ ...prev, [hev.id]: e.target.value }))}
                            ...
                            <div className="space-y-2 pt-2 border-t border-dark-border/30">
                              <span className="text-[9px] uppercase font-bold text-gray-500 block">Announcement History</span>
                              {(!hev.bulletinUpdates || hev.bulletinUpdates.length === 0) ? (
                                <p className="text-[10px] text-gray-600 italic">No announcements published yet.</p>
                              ) : (
                                <div className="space-y-2 max-h-36 overflow-y-auto no-scrollbar">
                                  {hev.bulletinUpdates.map((up) => (
                                    <div key={up.id} className="bg-dark-card/50 border border-dark-border/30 p-2.5 rounded text-[10px]">
                                      <div className="flex justify-between items-center text-[8.5px] text-neon-lavender font-mono font-bold mb-1">
                                        <span>{up.title}</span>
                                        <span>{up.date}</span>
                                      </div>
                                      <p className="text-gray-400 font-mono leading-normal">{up.content}</p>
                                    </div>
                                  ))}
                                </div>
                              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-md bg-dark-card border border-dark-border p-6 rounded-2xl shadow-neon relative space-y-4">
            <h2 className="text-lg font-black text-white flex items-center gap-2">
              <span>📣</span> {isEditing ? 'Edit Campus Event' : 'Host New Campus Event'}
            </h2>
            <p className="text-xs text-gray-400">
              {isEditing 
            ...
            <form onSubmit={handleHostNewEvent} className="space-y-4 font-mono">
              <div className="space-y-1">
                <label className="block text-[10px] uppercase font-bold text-gray-500">Event Title</label>
                <input
                  required
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="block text-[10px] uppercase font-bold text-gray-500">Category Type</label>
                  <select
                    value={newType}
                ...
                <div className="space-y-1">
                  <label className="block text-[10px] uppercase font-bold text-gray-500">Event Date</label>
                  <input
                    required
                    type="text"
              ...
              <div className="space-y-1">
                <label className="block text-[10px] uppercase font-bold text-gray-500">Location Venue</label>
                <input
                  required
                  type="text"
              ...
              <div className="space-y-1.5">
                <label className="block text-[10px] uppercase font-bold text-gray-500">Banner Image URL</label>
                <input
                  type="url"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="block text-[10px] uppercase font-bold text-gray-500">Ticket Type</label>
                  <select
                    value={newTicketType}
                    onChange={(e) => {
                      setNewTicketType(e.target.value as 'Free' | 'Paid');
                ...
                {newTicketType === 'Paid' && (
                  <div className="space-y-1">
                    <label className="block text-[10px] uppercase font-bold text-gray-500">Ticket Price ($)</label>
                    <input
                      required
                      type="number"
                      min="1"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="block text-[10px] uppercase font-bold text-gray-500">Total Capacity</label>
                  <input
                    required
                    type="number"
                    min="1"
              ...
              <div className="space-y-1">
                <label className="block text-[10px] uppercase font-bold text-gray-500">Brief Description</label>
                <textarea
                  required
      ...
      {selectedTicket && selectedEventForTicket && (
        <TicketModal
          ...
          ticketType={(selectedEventForTicket as HostedEvent).ticketType || 'Free'}
        />
      )}
    </div>
  );
